...
'use strict'
app.controller("borrowRequestController", function ($scope, $http, ngToast, stateService, $cookies) {
    ...
    // get_item in cart
    $http.get("api/cart/getborrowCart", config).then(function successCallback(response) {
        if (response.data.length > 0) {
            if (response.data[0].request_status == "borrow") {
    ...
    $scope.checkout = function (input) {
        swal({
            title: "Are you sure want to submit this item(s) as borrowed request?",
         ...
         function (isConfirm) {
             if (isConfirm) {
                 ...
                 $http.post("api/borrowRequest/", model, config).then(
                     function successCallback(response) {
                         myToastMsg = ngToast.success({
                             content: 'borrow request has been successful created.'
                         });
                         window.location.href = "/estore/createBorowRequest"
                     }, function errorCallback(response) {
                         myToastMsg = ngToast.danger({
                             content: 'borrow request fail ' + response.data
                         });
                     })
             }
    ...
    $http.get("api/borrowRequest/borrow_list", config).then(
        function successCallback(response) {
            //  console.log(response.data[0])
            $scope.borrowRqList = response.data
    ...
    $scope.updateAllocation = function () {
        ...
        .then(function successCallback(response) {
            ...
            $http.get("api/borrowRequest/" + orderid + "/orderp", config)
   .then(function successCallback(response) {
    ...
    $scope.deleteAllBorrowbyOrder = function (input) {
       ...
       function () {
           ...
           .then(function successCallback(response, status) {
               swal("Deleted!", "item has been deleted.", "success");
               window.location.href = "/estore/borrowRequestList"
           },
           function errorCallback(response) {
               swal("Cancelled", "item cannot be deleted: " + response.data, "error");
               myToastMsg = ngToast.danger({
    ...
    $scope.markAsFreshRq = function (input) {
        $http.get("api/borrowRequest/" + input + "/markAsFreshRq", config).
        then(function successCallback(response) {
            var myToastMsg = ngToast.info({
                content: 'Items has been successfully marked for processing.'
            });
            window.location.href = "/estore/borrowRequestList"
        },
        function errorCallback() {
    ...
    $scope.viewMorep = function (input) {
        ...
        //var orderid = sessionStorage.getItem('orderid');
        // alert(orderid)
        window.location.href = "/estore/borrowed_items_by_order";
    }
    var orderid = sessionStorage.getItem('orderid');
    $http.get("api/borrowRequest/" + orderid + "/orderp", config)
    .then(function successCallback(response) {
